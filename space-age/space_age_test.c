...
void test_age_on_earth(void) {
    enum planet current_planet = EARTH;
    int current_seconds = 1000000000;
    float result_age = age(current_planet, current_seconds);
    // --- Debug Print Statements (optional, keep if you're using them) ---
    printf("\n--- Test: Age on Earth ---\n");
    printf("Input: Planet = %d (EARTH), Seconds = %d\n", current_planet, current_seconds);
    printf("Result from age() function: %.2f\n", result_age);
    // ----------------------------
    TEST_ASSERT_FLOAT_WITHIN(0.01, 31.69, result_age);
}
